...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["ny_medlem"])){
    ...
    $query = "INSERT INTO medlem (m_nr, fornavn, etternavn, adresse, postnr, poststed, fodt, telefon, mail, betalt) VALUE
    (:m_nr, :fornavn, :etternavn, :adresse, :postnr, :poststed, :fodt, :telefon, :mail, :betalt)";
    ...
    <main>  
        <header>
            <h1>
                <?php
                    if($stmt){
                        echo "Medlem Ble Registerert!";
                    }
                    else{
                        echo "Error, Medlem Ble Ikke Registerert!";
                    }
                ?>
            </h1>
        </header><br><br>
